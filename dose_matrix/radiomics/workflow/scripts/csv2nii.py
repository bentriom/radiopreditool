...
def to_nii(path_csv, path_nii, list_csv_files):
    ...
    if test_xyz_nan:
        ...
        logger.warn(f"{first_newdosi_file}: has NaN values in X, Y or Z. Dropping {nbr_rows_before - nbr_rows_after} rows.")
    ...
    for i in idx_sort_by_date_csv_files[1:]:
        ...
        if delta_time.total_seconds() > 6*30*24*3600:
            ...
            if test_xyz_nan:
                ...
                logger.warn(f"{current_newdosi_file}: has NaN values in X, Y or Z. Dropping {nbr_rows_before - nbr_rows_after} rows.")
            if df_dosi.shape[0] != df_other_dosi.shape[0]:
                logger.warn(f"{first_newdosi_file} and {current_newdosi_file}: rows numbers are different. Stopping the sum.")
                break
            if df_other_dosi.shape[0] <= 1:
                logger.warn(f"{current_newdosi_file}: has <= 1 rows. Stopping the sum.")
                break
            well_ordered_rows = check_summable_df(df_dosi, df_other_dosi)
            if well_ordered_rows:
                df_dosi['ID2013A'] += df_other_dosi['ID2013A']
            else:
                logger.warn(f"{first_newdosi_file} and {current_newdosi_file}: rows are not well ordered.")
                for col in int_cols:
                ...
                if not check_summable_df(df_dosi, df_other_dosi):
                    logger.warn(f"{first_newdosi_file} and {current_newdosi_file}: same rows number but different. Stopping the sum.")
                    break
                df_dosi['ID2013A'] += df_other_dosi['ID2013A']
    ...
    if df_dosi.shape[0] <= 1:
        logger.warn(f"{first_newdosi_file}: has <= 1 rows. Creating empty nii files.")
