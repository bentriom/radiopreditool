...
multiple_scores_cox_radiomics <- function(nb_estim, screening_method, event_col, analyzes_dir, duration_col) {
    stopifnot({
      nb_estim > 0
    ...
    dir.create(paste0(analyzes_dir, "coxph_R/"), showWarnings = FALSE)
    nworkers <- get.nworkers()
    cox_logfile <- paste0(analyzes_dir, "multiple_scores_cox_lasso_radiomics_R_",screening_method,".log")
    if (file.exists(cox_logfile)) { file.remove(cox_logfile) }
    log_appender(appender_file(cox_logfile, append = TRUE))
...
multiple_scores_cox_bootstrap_radiomics <- function(nb_estim, screening_method, event_col, 
                                                    analyzes_dir, duration_col, n_boot) {
    stopifnot({
      nb_estim > 0
      screening_method %in% c("all", "features_hclust_corr")
    })
    dir.create(paste0(analyzes_dir, "coxph_R/"), showWarnings = FALSE)
    nworkers <- get.nworkers()
    cox_logfile <- paste0(analyzes_dir, "multiple_scores_cox_bootstrap_lasso_radiomics_R_",screening_method,".log")
    if (file.exists(cox_logfile)) { file.remove(cox_logfile) }
    log_appender(appender_file(cox_logfile, append = TRUE))
    ...
    # features <- `if`(file_features == "all", colnames(df_dataset), as.character(read.csv(file_features)[,1]))
    # # Add "X" for R colname compatibility
    # features <- as.character(lapply(features, function(x) { `if`(str_detect(substr(x, 1, 1), "[0-9]"), 
    #                                                              paste("X", x, sep = ""), x) }))
    clinical_vars <- get.clinical_features(features, event_col, duration_col)
...
if (length(args) > 1) {
    ...
    if (run_type == "baseline_models") {
        ...
        multiple_scores_cox_radiomics(nb_estim, "all", event_col, analyzes_dir, duration_col)
        ...
        print(paste("n_boot:", n_boot))
        multiple_scores_cox_bootstrap_radiomics(nb_estim, "all", event_col, analyzes_dir, duration_col, n_boot)
    } else if (run_type == "cox_lasso_radiomics_features_hclust_corr") {
        multiple_scores_cox_radiomics(nb_estim, "features_hclust_corr", event_col, analyzes_dir, duration_col)
