...
selection.coxnet <- function(formula, data, alpha = 1, nfolds = 5, list.lambda = NULL,
    ...
    log_info(paste("Number of covariates", length(covariates)))
    log_info(paste("Number of non null covariates", length(nonnull.covariates)))
    log_info(paste0("Best lambda: ", best.lambda, "(lambda.1se coxnet:", coxnet_model$lambda.1se,
    ...
    if (is.null(selected_features)) {
        ...
        if (boot.parallel == "boot.multicore") {
          ...
          resBoot <- foreach(i = 1:B, .combine = 'rbind', .export = functions_to_export,
                             .packages = c("survival")) %do% {
            idx_boot <- sample(nrow(data), replace = T)
            sample.selection.coxnet(data, idx_boot, lasso_data_full, formula, alpha,
                                    best.lambda.method, nfolds, type.measure, pred.times, run_type)
          }
          if (nrow(resBoot[,-c(1,2)]) != B)
            stop(paste("Bootstrap matrix nrow:", nrow(resBoot[,-c(1,2)]), "- B:", B))
...
model_cox <- function(df_trainset, df_testset, covariates, event_col, duration_col,
    ...
    log_info(paste0("(", id_set, ") ", "Trainset: ", nrow(df_model_train), " samples"))
    log_info(paste0("(", id_set, ") ", "Testset: ", nrow(df_model_test), " samples"))
    stopifnot(nrow(df_model_train) > 1)
    stopifnot(nrow(df_model_test) > 1)
    stopifnot(sum(df_model_train[event_col]) > 0)
    # Z normalisation
    ...
    if (penalty == "none") {
        ...
        if (load_results) {
            ...
            if (save_results) {
                mat.coefs <- data.frame(t(as.matrix(coef(coxmodel$coxnet.fit$glmnet.fit))))
    ...
    if (save_results) {
        ...
        if (!is.null(best_coefs)) {
          ...
          file.create(paste0(save_results_dir, "best_coefs.csv"))
        }
...
plot_cox_coefs <- function(save_results_dir) {
  file_df_coefs <- paste0(save_results_dir, "best_coefs.csv")
  if (file.size(file_df_coefs) ==  0L) {
    ggplot() + theme_void() + geom_text(aes(0,0, label = 'No non null coefficients.')) + xlab(NULL)
  } else {
...
plot_cox_lambda_path <- function(save_results_dir) {
  ...
  file_df_coefs <- paste0(save_results_dir, "best_coefs.csv")
  names.nonnull.coefs <- NULL
  if (file.size(file_df_coefs) > 0L) {
    df_best_coefs <- read.csv(file_df_coefs)
...
plot_bootstrap_cox <- function(save_results_dir, B) {
  ...
  df_freq_covar = data.frame(labels = pretty.labels(names(freq_selection)), coefs = freq_selection)
  df_freq_covar = df_freq_covar[order(-df_freq_covar$coefs), ]
  if (nrow(df_freq_covar) > 30) df_freq_covar <- df_freq_covar[1:30, ]
  ggplot(df_freq_covar, aes(x = reorder(labels, coefs), y = coefs)) + geom_bar(stat = "identity") +
