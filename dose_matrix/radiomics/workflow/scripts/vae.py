...
class CVAE_3D(nn.Module):
    ...
    def __init__(self, image_channels = 1, z_dim = 32, input_image_size = None):
        ...
        output_image_shape = [int(v) for v in self.input_image_size - 5*3]
        h_dim = 128 * np.prod(output_image_shape) # - nb_conv3D*(kernel_size-1)
        self.h_dim = h_dim
        ...
        # Decoder part
        unflatten_size = [128] + output_image_shape
        self.unflatten_size = unflatten_size
        print(unflatten_size)
